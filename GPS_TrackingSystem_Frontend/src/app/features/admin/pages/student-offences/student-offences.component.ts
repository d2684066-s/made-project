...
export class StudentOffencesComponent implements OnInit {
  ...
  loadOffences() {
    ...
    this.apiService.getStudentOffences().subscribe({
      next: (data) => {
  ...
  deleteOffence(id: string) {
    if (confirm('Are you sure you want to delete this offence?')) {
      this.apiService.deleteStudentOffence(id).subscribe({
        next: () => {
          this.loadOffences();
  ...
  markAsPaid(offence: any) {
    if (confirm('Mark this offence as paid?')) {
      this.apiService.markStudentOffenceAsPaid(offence.id).subscribe({
        next: () => {
          this.loadOffences();
